...
| POST | `/v1/ifsb-regulatory-report-procedure/initiate` | Initiate |
| GET | `/v1/ifsb-regulatory-report-procedure` | Retrieve (list) |
| GET | `/v1/ifsb-regulatory-report-procedure/{crId}/retrieve` | Retrieve |
| PUT | `/v1/ifsb-regulatory-report-procedure/{crId}/update` | Update |
| PUT | `/v1/ifsb-regulatory-report-procedure/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/ifsb-regulatory-report-procedure/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t isb/isb-ifsb-regulatory-reporting:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install isb-ifsb-regulatory-reporting ./helm -n isb-operations
```

Exposed through the platform Gateway at path prefix `/isb-ifsb-regulatory-reporting` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
